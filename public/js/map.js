...
async function loadMap(mode) {
    mode = mode || 'member';
    document.querySelectorAll('#page-cartographie .course-filter').forEach(b => b.classList.remove('active'));
    ...
    if (mode === 'company') {
        data.forEach(c => {
            ...
            L.marker([c.lat, c.lng], {icon}).addTo(mapInstance)
                .bindPopup(`<div style="font-family:Inter,sans-serif"><strong style="font-size:16px;color:#1a2d4d">${c.company}</strong><br><span style="color:#6b7280">${c.member_count} membre(s)</span><ul style="margin:8px 0 0 16px;font-size:13px">${members}</ul></div>`);
        });
        ...
        data.forEach(m => {
            ...
            const color = colors[m.sector] || '#1a2d4d';
            const icon = L.divIcon({
            ...
            L.marker([m.lat, m.lng], {icon}).addTo(mapInstance)
                .bindPopup(`<div style="font-family:Inter,sans-serif;min-width:200px">
                    <strong style="font-size:15px;color:#1a2d4d">${m.first_name} ${m.last_name}</strong> ${badges}<br>
                    ...
                    ${authToken ? `<button onclick="startConversation(${m.id},'${m.first_name} ${m.last_name}')" style="margin-top:8px;background:#1a2d4d;color:#fff;border:none;padding:4px 12px;border-radius:4px;cursor:pointer;font-size:12px">Contacter</button>` : ''}
                </div>`);
        });
...
function filterCourses(btn, cat) {
    document.querySelectorAll('#page-formations .course-filter').forEach(b => b.classList.remove('active'));
